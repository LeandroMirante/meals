...
class _TabsScreenState extends State<TabsScreen> {
      ...
      final result = await Navigator.of(context).push<Map<Filter, bool>>(
        MaterialPageRoute(
          builder: (ctx) => FiltersScreen(
            currentFilters: _selectdFilters,
          ),
        ),
      );
      setState(() {
  ...
  Widget build(BuildContext context) {
    final availableMeals = dummyMeals.where((meal) {
      ...
      if (_selectdFilters[Filter.lactoseFree]! && !meal.isLactoseFree) {
        return false;
      }
      if (_selectdFilters[Filter.vegetarian]! && !meal.isVegetarian) {
        return false;
      }
      if (_selectdFilters[Filter.vegan]! && !meal.isVegan) {
        return false;
      }
      return true;
    }).toList();
